    ...
    if( serial_buffer[start] == 0xAA )
    {
      ...
      if( ( ( (unsigned short)serial_buffer[checksum_0] << 8 ) | serial_buffer[checksum_1] ) == sum )
      {
        ...
        for( int i = 0; i < LEDS; i++ )
        {
          ...
          if(serial_buffer[idx] == serial_buffer[idx+1] &&  serial_buffer[idx+1] == serial_buffer[idx+2]){
             strip.setPixelColor(i, 0,0,0,serial_buffer[idx]);
           ...
           int idx = start + 1 + ( 3 * 40 );
  
          if( idx >= (PACKET_SZ - 1) )
          {
            idx = idx - PACKET_SZ;
          }
             analogWrite(r, serial_buffer[idx+0]);
             analogWrite(g, serial_buffer[idx+1]);
             analogWrite(b, serial_buffer[idx+2]);
